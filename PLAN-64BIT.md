...
| **T-A3** | **Thermal HAL 2.0** — ✅ **SELESAI & TERBUKTI DI PERANGKAT 14 Sep 2026** (`ea262bb`) — `HAL Ready: true`, 8 suhu mengalir, 4 listener menerima data | `5646c17` | **tidak ada HAL thermal sama sekali** (0 sebutan di `device.mk`); `dumpsys thermalservice` memuat 6 listener menunggu data yang tak pernah datang | ⚠️ Config a6010 **tidak boleh disalin mentah**: `bms` sebagai `SKIN` cuma naik 0,8 °C di bawah beban → status global terkunci `NONE` selamanya. Ditukar ke `pm8916_tz`, dan `SHUTDOWN` dicabut di semua sensor proxy — lihat [`plan-64bit/t-a3/`](plan-64bit/t-a3/) | 1–2 jam → **~3 jam** |
| **T-A4** | **Widevine DRM L3** — ⚠️ **SEBAGIAN** 14 Sep 2026 (device tree `3b4a37d`, vendor `3978bf9`) — CDM 14.0.0 L3 hidup dan menulis device certificate, tetapi **EME browser ditolak**: `DrmHalHidl.cpp:529` mewajibkan `IDrmFactory@1.2` dan blob ini `@1.1` | `8f4a42d`, `0af1a56` | 0 sebutan di `device.mk`, padahal blobnya sudah ada di vendor | ⚠️ **Ukur dulu terbayar**: protobuf 3.9.1 pohon ini kehilangan **12 dari 55** simbol (25.8: 21), jadi harapan "mungkin nol" gugur — tetap perlu 3.0.0 dari sumber. Lebih penting: linker `[legacy]` satu-namespace **membatalkan dua keputusan 23.2** — `drm@1.1.vendor` tak perlu, dan `install_symlink` justru **tidak cukup** karena `/system/lib` dicari lebih dulu; diganti `stem` + `setenv LD_LIBRARY_PATH`. Lihat [`plan-64bit/t-a4/`](plan-64bit/t-a4/) | 2–8 jam |
...
| **T-B3** | **Power HAL: `POWER_HINT_LOW_POWER` + `SUSTAINED_PERFORMANCE`** — ✅ **SELESAI 14 Sep 2026** (`ba712d2`) | `ad6cd94`, `65bf56d`, `79745e2` | Ketiganya diambil sebagai satu kesatuan. Nilai 1209600 **diukur ulang di ROM ini**: beban 4 inti 3 menit, `scaling_max_freq` tak pernah turun, suhu mendatar di 66 °C. Diverifikasi pada varian **lib64** yang benar-benar dimuat. [`plan-64bit/tier-b/`](plan-64bit/tier-b/) |
| **T-B4** | **WireGuard** — ✅ **SELESAI & TERBUKTI JALAN 14 Sep 2026** (`ba712d2`) | `1dda093` + kernel `be67c444` | Kernel terbukti hidup (`ip link add type wireguard` berhasil), alat userspace yang hilang. Diuji ujung ke ujung di perangkat: `wg show` mengembalikan public key yang cocok dengan `genkey \| pubkey`. [`plan-64bit/tier-b/`](plan-64bit/tier-b/) |
| **T-B5** | **Daemon IMS bisa dieksekusi lagi** — ⛔ **TERHALANG 14 Sep 2026**: binernya **tidak ada di cabang mana pun** repo vendor kita (`vendor/bin` hanya 11 biner, nol IMS; yang ada cuma pustaka `lib64`). Premisnya berasal dari vendor tree proyek 23.2. [`plan-64bit/tier-b/`](plan-64bit/tier-b/) | temuan Fase 0 23.2 | `imsqmidaemon` (120.464 B) dan `imsdatadaemon` adalah **ELF 64-bit**; di ROM 32-bit keduanya mustahil dijalankan karena hanya ada `/system/bin/linker` 32-bit. Build 64-bit menyediakan `linker64` dan **penghalang arsitekturnya hilang**. ⚠️ Ini **bukan** berarti VoLTE hidup: `ims.apk` bermasalah karena alasan lain (`System.arraycopy` menjadi privat sejak Android 11 — berlaku penuh di A13). Kerjakan **setelah** ROM terbukti boot |
| **T-B6** | Penyetelan audio/GPU — ⚠️ **2 DARI 4, 14 Sep 2026** (`ba712d2`) | `2c3ad97`, `9270131`, `08bda34`, `bc04b73` | ✅ volume speaker, tetapi di **`mixer_paths_mtp.xml`** — sound card `msm8x16-snd-card-mtp` dipetakan ke sana (`platform.c:838`), jadi menyunting `mixer_paths.xml` seperti 23.2 akan **no-op**. ✅ GPU `default_pwrlevel` 1→2. ❌ `ccodec` **gugur**: A13 masih punya 37 codec termasuk decoder audio. ❌ MULTI_VOICE_SESSIONS **terbukti tidak diperlukan**: panggilan diuji 14 Sep 2026, speaker dan mic berfungsi, dan log menunjukkan parameter `vsid` **sampai** ke audio HAL — mekanisme yang persis rusak di 23.2 karena stub `libmedia` mereka. Menerapkannya di sini justru akan merusak. [`plan-64bit/tier-b/`](plan-64bit/tier-b/) |
| **T-B7** | Perbaikan kecil yang berdiri sendiri — ⚠️ **2 DARI 3, 14 Sep 2026** (`ba712d2`) | `accf67b`, `e0b82e5`, `fac5f72` | ✅ PinnerService: **5 dari 9** entri salah (bukan 4); dua entri dalvik-cache **dibuang** karena diturunkan otomatis (`PinnerService.java:315-336`). Ongkosnya diukur: 68,8 → ~150 MB ter-pin. ✅ hung task timeout 0→90, `panic` sengaja tak disentuh. ❌ ueventd `idle_time` **gugur**: composer sehat, nol tombstone. [`plan-64bit/tier-b/`](plan-64bit/tier-b/) |
