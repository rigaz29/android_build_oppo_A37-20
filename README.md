...
| [`plan-64bit/tier-a/`](plan-64bit/tier-a/) | **Tier A: T-A1, T-A2, T-A5, T-A7 selesai** — lmkd ke PSI, zram 768 MB, TRIM, FMRadio dibuang |
...
| [`plan-64bit/temuan-video/`](plan-64bit/temuan-video/) | **Diagnosis: rekaman terkunci 720p** — `LegacyMetadataMapper` membuang 1920x1080 karena foto terbesar 4:3. Diperbaiki lewat patch `0030`, belum diuji |
| [`plan-64bit/temuan-gapps/`](plan-64bit/temuan-gapps/) | **Diagnosis: MindTheGapps ditolak** — TWRP 32-bit, dan `/tmp` berbasis RAM terlalu kecil untuk paket 950 MB |
| [`plan-64bit/temuan-bootwatchdog/`](plan-64bit/temuan-bootwatchdog/) | **Watchdog menjatuhkan boot sehat setelah GApps** — `odsign` bukan sinyal kompilasi yang cukup; diperbaiki dengan deteksi `dex2oat` |
| [`plan-64bit/audit-log/`](plan-64bit/audit-log/) | **Audit log: bug tanpa gejala** — dua cacat milik saya sendiri diperbaiki, satu cacat kernel lama ditemukan |
| [`plan-64bit/baseline-deepsleep/`](plan-64bit/baseline-deepsleep/) | **Baseline deep sleep** — 8 jam idle: 99,1% suspend, ~1%/jam, proyeksi standby ~4 hari |
...
| [`plan-64bit/penyetelan-mm/`](plan-64bit/penyetelan-mm/) | **Tiga penyetelan memori** — dua dikerjakan (pgscan datar untuk lmkd, force-SCAN_ANON bersyarat); PELT ditahan karena tabel patch sumbernya salah hitung |
| [`plan-64bit/temuan-lmkd-membunuh-foreground/`](plan-64bit/temuan-lmkd-membunuh-foreground/) | **lmkd membunuh aplikasi di layar** — Settings dibunuh pada adj=0 dengan 1 GB cache menganggur; sebabnya `ro.lmk.use_new_strategy=false` |
...
| [`plan-64bit/ebpf/`](plan-64bit/ebpf/) | **eBPF bekerja di perangkat** — verifier menerima seluruh program Android, `xt_bpf` rev 1 menghidupkan Data Saver, galat `BpfHandler` 39 → 0 |
| [`plan-64bit/cgroup-bpf/`](plan-64bit/cgroup-bpf/) | **cgroup-BPF, sedang dikerjakan** — backport penuh cgroup v2 (~400 KB, mengganti inti) ternyata TIDAK perlu; permukaan minimalnya 15 galat di satu berkas |
...
| [`plan-64bit/fase-0/`](plan-64bit/fase-0/) | **Fase 0 selesai** — 323 entri blob terpilah per arsitektur (71 dual, 159×32, 43×64), closure RenderScript terukur, dan dua klaim rencana induk yang dikoreksi |
